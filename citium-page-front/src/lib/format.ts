 ...
 */
export function formatPrice(value?: string | number, notPricedText: string = 'Precio a consultar'): string {
    if (value == null || value === '') return notPricedText;
    
    const text = String(value).trim();
    
    // Check if the entire price is 0
    if (text === '0' || text === '0.0') return notPricedText;
    ...
    return text
        ...
        .map(part => {
            ...
            if (isNaN(num) || num === 0) {
                // if parsing fails or number is 0, return placeholder
                return notPricedText;
            }
