...
export async function getStrapiData(url: string): Promise<any> {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Strapi response error: ${response.status}`);
            }
...
export const propertyService = {
    ...
    getProperties: async (filters?: {
        ...
        try {
            ...
            const meta = json.meta || { pagination: { page, pageSize, pageCount: 1, total: properties.length } };
                        ...
                        // Apply client-side sorting for price fields
            const sortType = filters?.sort || 'newest';
            if (sortType === 'highToLow' || sortType === 'lowToHigh') {
                properties.sort((a, b) => {
                    // Extract numeric price from priceSale or priceRent
                    const getPriceValue = (prop: Property): number => {
                        const priceStr = prop.priceSale || prop.priceRent || '0';
                    ...
                    return sortType === 'highToLow' ? priceB - priceA : priceA - priceB;
                });
            } else {
    ...
    getPropertyById: async (id: string): Promise<Property | null> => {
        try {
            ...
            const res = await fetch(`${BASE_URL}/api/properties/${id}?populate=*`, {
                cache: 'no-store'
            });
